...
- **A hero card at the top of Settings** with your version, your plan, and a way back to the
  release notes.
- **A 1:1 hold** that tells Doraemon to leave an agent alone.
- **Editable agent names,** and Doraemon can choose a worker's avatar and accent when he hires.
...
- [#159](https://github.com/chaitanyagiri/munder-difflin/pull/159) [@gpechieu](https://github.com/gpechieu): reliable spawn, teardown and floor cards for the workers Doraemon hires
...
- [#219](https://github.com/chaitanyagiri/munder-difflin/pull/219) [@chaitanyagiri](https://github.com/chaitanyagiri): engine availability is checked before Doraemon's engine is committed
...
- Agent selection is visible on every card including Doraemon's — it used to be drawn in each
...
- **Prerequisites** (Settings) — live status for uv, git, Node, MemPalace and every agent engine,
  with real paths, platform-correct install commands, and a button that asks Doraemon to fill the
  gaps.
...
- One card size for every agent; Doraemon is distinguished by surface, not by a heavier border.
...
**A new brand mark: Doraemon's portrait replaces the "MD" tile.**
...
### Changed
- **Doraemon is your clone.** Onboarding refers to him as your clone throughout, and his card on
...
- **Compact Command Center header.** At sidebar width the old header wrapped its
  ...
  CENTER** title + "Doraemon runs the floor" subtitle (both ellipsize), and the floor
...
**The queue you can trust, a Doraemon who actually knows the floor, and an IDE that shows
...
first-party features — **voice Doraemon with live floor context + full app control**,
...
- **Talk mode grows up: live context + full control.** Doraemon's voice session now opens
...
- **Remote Control sessions are named after the agent** ([#81](https://github.com/chaitanyagiri/munder-difflin/pull/81)). claude.ai / the mobile app now shows "Doraemon", "Jim", … instead of `<hostname>-<random>`, so a floor full of RC sessions is finally tellable-apart. *(gts-47)*
...
**Talk to Doraemon.** The headline is **Realtime Doraemon** — a low-latency **voice channel to the
GOD orchestrator**, running alongside the async terminal floor. Press **Talk**, and Doraemon listens,
...
**doraemon-voice** actor that pings the GOD terminal. He greets you on connect, **speaks task
...
> OpenAI key — connect → mic → Doraemon answers via the read tools, and the full destructive action
...
### Added
- **Realtime Doraemon — talk to the GOD orchestrator by voice.** A new low-latency realtime channel
  (OpenAI Realtime API over WebRTC) sits next to the async terminal. A **Talk** toggle (on Doraemon's
  card and in any fullscreen terminal) opens a mic session with EC/NS/AGC, semantic-VAD turn-taking +
  barge-in, and a device picker for both microphone and speaker. Doraemon runs his own persona and
...
- **Voice action set with tiered echo-back confirmation.** Doraemon can *do* things by voice — read
...
- **"Respond when done" completion loop.** Voice-dispatched work reports back on its own: a
  ...
  reply in the inbox) and **proactively pushes the event into the live session so Doraemon speaks it
  ...
  `CompletionToast.tsx`).
- **doraemon-voice as a distinct actor.** Actions taken by voice are attributed to a separate
  **doraemon-voice** identity in messages, the board, and the activity log, and notify the GOD PTY —
...
- **Greeting on connect.** When a session goes live, Doraemon opens with a warm, rotating greeting
...
- **Conversational read-layer.** The voice read tools were reworked to actually answer hive
  ...
  Doraemon talk through roster, tasks, and floor state naturally.
- **Voice read-layer over hive messages (read/brief-only).** Realtime Doraemon can now read message
...
- **Talk reachable from any fullscreen terminal.** The Talk toggle is no longer Doraemon-only chrome —
  it's reachable in any fullscreen terminal view (the toggle is global/session state, so it's correct
  everywhere), while the per-session cost HUD stays Doraemon-only (`FullscreenTerminal.tsx`).
- **OpenAI Realtime key — documented and gated.** **Settings → AI Engines** now documents the
  **OpenAI Realtime key** as its own requirement (the same OpenAI provider key, distinct from your
  ...
  never lands on a silently-dead button (`RealtimeDoraemonToggle.tsx`, `SettingsModal.tsx`,
...
worker *and* as Doraemon, with **bring-your-own keys + local LLMs**. Plus two reliability fixes: the
...
every hire — and Doraemon himself — a pluggable engine (Claude Code / Antigravity / Codex /
...
write-only, registry-driven Settings flow. Doraemon can now **spawn an ephemeral worker straight
...
- **Swappable Doraemon (god) engine.** The orchestrator is no longer hard-wired to one CLI: `useHive` gains an engine-spawn path, Onboarding gains an **engine picker** for Doraemon, and a **change-engine flow** lets you re-home the god orchestrator onto a different engine without rebuilding the floor.
...
- **All human dispatch flows through the god.** Every Command Center dispatch now mails the god (`Task from the human`) instead of writing straight into a worker's inbox; the worker dropdown becomes a **suggested owner** (Doraemon still decides), so nothing skips the orchestrator. (Thanks @Gulum — #45, fixes #44.)
...
- **Heartbeat is inbox-driven.** The floor heartbeat (`reengageGod`) no longer types directly into Doraemon's terminal; it drops its digest in his inbox, which the busy-aware inbox-wake delivers once he's idle.

### Docs
- Expanded the README roadmap (chat integrations, pluggable agent CLIs, realtime Doraemon).
...
### Added
- **Command Center overhaul.** Doraemon's control surface was reworked into the place you actually run the floor from — the roster, dispatch, schedules, memory, and activity views now carry the new live signals (token budgets, telemetry, breaker state) without becoming a wall of numbers.
...
### Added
- **Dwight, Doraemon's prep assistant.** A persistent, visible assistant agent
  (Sonnet, 1M context) spawns on startup. A global **enrich** toggle routes
  Doraemon's queued prompts through Dwight first — he gathers repo context and
  rewrites the prompt, then forwards it to Doraemon through the hive; toggle it off
  and prompts go straight to Doraemon.
- **Doraemon's Command Center.** His sidebar becomes a control surface with
...
- **Settings panel** (title-bar gear) with a **Reset & start over** action that wipes
  Doraemon's memories, the entire hive (every agent, message, task, and the board), and
...
- Doraemon no longer marches to the door flagged "needs you" right after finishing a turn —
