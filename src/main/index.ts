...
// Doraemon reads + the breaker beat, so guardrails + monitoring work even when the
...
/** Build + write the live fleet snapshot Doraemon reads (`<hive>/fleet.json`).
...
// ─── Slack webhook server (Slack message → Doraemon's queue) ──────────────────
...
5. REPORT TO GOD — the agent then tells you (Doraemon) what it did.
...
function createWindow(opts: { floor?: boolean } = {}): BrowserWindow {
  ...
  // Realtime Doraemon voice session (`realtimeVoiceEnabled`, flipped on by the
...
async function spawnAgentCore(opts: AgentSpawnOptions, owner: Electron.WebContents | null): Promise<{ ok: boolean; error?: string; cwd?: string; worktreePath?: string; resumeNotFound?: boolean; resumed?: boolean; seedPrompt?: string }> {
  ...
  if (opts.hive && claudeProvider) {
    ...
    // Name the Remote Control session after the agent (Doraemon, Jim, Dev1…) so it
...
ipcMain.handle('app:resetAll', () => {
  ...
  // Erase the hive (Doraemon's + every agent's memory, inboxes, tasks, board,
...
// (Realtime Doraemon's get_agent_detail / list_agents). One read that joins
...
// is live-only) so Doraemon can speak to inactive agents — their cwd and memory
...
// ─── IPC: Realtime Doraemon (voice orchestrator — ephemeral token mint, rt-1) ──
...
// ─── IPC: Realtime Doraemon voice ACTIONS (rt-5, Phase 2) ─────────────────────
...
// doraemon-voice attribution — lives in ./realtimeActions. This site only injects
// the existing functions; it adds NO new orchestration logic.
// ─── IPC: Realtime Doraemon completion watcher (rt-12, Phase 2) ───────────────
// Jim's net-new engine (realtimeCompletionWatcher.ts) detects a voice-dispatched
// task finishing (card→done OR a done-reply in doraemon-voice's inbox) and EMITS it;
// I own the seam — inject the hive read deps, push completions to the live session
// (so Doraemon speaks them unprompted), and bridge waitFor / queue-drain over IPC.
const completionWatcher = initCompletionWatcher({
  ...
  // Voice dispatches go out as from:doraemon-voice, so assignee done-replies land here.
  readInbox: () => {
    // Voice dispatches go out from:doraemon-voice, so done-replies normally land in its
    ...
    try {
      const dv = hive.inbox('doraemon-voice') as unknown as InboxMessage[];
      const godId = hive.registry().godId;
      const god = godId ? (hive.inbox(godId) as unknown as InboxMessage[]) : [];
      const seen = new Set<string>();
      return [...dv, ...god].filter((m) => !!m?.id && !seen.has(m.id) && seen.add(m.id) !== undefined);
    } catch {
      return [];
    }
  },
  onNotify: (evt) => { try { if (Notification.isSupported()) new Notification({ title: 'Doraemon', body: evt.summary }).show(); } catch { /* best-effort */ } }
});
 ...
 *  fleet snapshot Doraemon reads (~8s) + the breaker/cost-ledger beat (~30s).
...
app.whenReady().then(() => {
  // Realtime Doraemon mic-gate hygiene (rt-8 / Pam rt-10 nit): the voice session
