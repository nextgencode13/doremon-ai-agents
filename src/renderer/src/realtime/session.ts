/**
 * Realtime Doraemon — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * get_config / get_memory / get_activity) and god's rt-6 "Doraemon" persona, so the
 ...
 * loop at a time) exposed through a `useRealtimeDoraemon()` hook via useSyncExternalStore.
 *
 * Branch feat/realtime-doraemon. See board.md "🎙 REALTIME DORAEMON".
 */
...
export interface RealtimeDoraemonState {
...
/** Warm openers Doraemon leads with the moment a voice session connects, so he
...
/** Doraemon's voice persona (rt-6 — the final Phase-1 instructions, authored by god). Doraemon
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const DORAEMON_PERSONA =
  `You are Doraemon — the voice of the orchestrator ("god") of a hive of autonomous Claude coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
HIVE VOCABULARY. Agents have names like "Nobita", "Shizuka", "Gian", "Suneo", "Dorami", "Dekisugi", or "Doraemon"; refer to them by name. "god" is the floor orchestrator whose voice you are. A card's status is todo, doing, blocked, or done. The circuit breaker is healthy, or steering an agent that's looping or idle. Blocked usually means waiting on the human.
...
let state: RealtimeDoraemonState = {
  status: 'off',
  error: null,
  muted: false,
  model: null,
...
/** The <audio> sink for Doraemon's voice. */
...
function setState(patch: Partial<RealtimeDoraemonState>): void {
  state = { ...state, ...patch };
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Doraemon's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Doraemon's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Doraemon
    ...
    // Floor deltas — silent appends that keep Doraemon's picture live without
    ...
    // subscribe so a detected completion makes Doraemon speak it unprompted.
    ...
    // Open the conversation: have Doraemon speak a warm greeting as his first turn
...
function getSnapshot(): RealtimeDoraemonState {
  return state;
}

/**
 * React binding for the Realtime Doraemon voice loop. Returns the current state plus
 ...
 */
export function useRealtimeDoraemon(): RealtimeDoraemonState & {
