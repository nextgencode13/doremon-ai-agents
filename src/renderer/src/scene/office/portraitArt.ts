...
// Nobita: neat round black bowl haircut
const styleBowl: HairFn = (buf, color, skinBase) => {
  const [hi, base] = shades(color);
  rect(buf, HX0, 2, HX1, 4, base);
  for (let x = HX0 - 1; x <= HX1 + 1; x++) set(buf, x, 3, base);
  rect(buf, HX0 - 1, 4, HX1 + 1, 6, base);
  for (let x = HX0; x <= HX1; x++) set(buf, x, 7, base);
  for (let y = 6; y < 10; y++) { set(buf, HX0 - 1, y, base); set(buf, HX0, y, base); set(buf, HX1, y, base); set(buf, HX1 + 1, y, base); }
  for (let x = HX0; x <= HX1; x++) if (alphaAt(buf, x, 2)) set(buf, x, 2, hi);
  for (const x of [7, 8, 9, 10]) set(buf, x, 6, hi);
};

// Shizuka: neat bangs with twin side pigtails and pink ribbons
const stylePigtails: HairFn = (buf, color, skinBase) => {
  const [hi, base] = shades(color);
  rect(buf, HX0, 2, HX1, 4, base);
  for (let x = HX0 - 1; x <= HX1 + 1; x++) set(buf, x, 3, base);
  rect(buf, HX0 - 1, 4, HX1 + 1, 6, base);
  ...
  for (let x = 6; x < 12; x++) set(buf, x, 6, base);
  for (let y = 6; y <= 16; y++) {
    set(buf, HX0 - 2, y, base); set(buf, HX0 - 1, y, base);
    set(buf, HX1 + 1, y, base); set(buf, HX1 + 2, y, base);
  }
  const ribbon: RGB = [245, 100, 145];
  set(buf, HX0 - 2, 7, ribbon); set(buf, HX0 - 1, 7, ribbon);
  set(buf, HX1 + 1, 7, ribbon); set(buf, HX1 + 2, 7, ribbon);
  for (let x = HX0; x <= HX1; x++) if (alphaAt(buf, x, 2)) set(buf, x, 2, hi);
};

// Suneo: sharp forward-thrust side spikes
const styleSuneo: HairFn = (buf, color) => {
  const [hi, base] = shades(color);
  rect(buf, HX0, 3, HX1, 5, base);
  for (let x = HX0 - 1; x <= HX1 + 1; x++) set(buf, x, 4, base);
  set(buf, 3, 2, base); set(buf, 2, 1, base); set(buf, 1, 0, hi);
  set(buf, 4, 1, base); set(buf, 5, 2, hi);
  set(buf, 7, 2, base); set(buf, 8, 1, base); set(buf, 9, 0, hi);
  set(buf, 11, 2, base); set(buf, 12, 1, base); set(buf, 13, 1, hi);
  for (let y = 5; y < 9; y++) { set(buf, HX0 - 1, y, base); set(buf, HX1 + 1, y, base); }
  for (let x = HX0; x <= HX1; x++) set(buf, x, 5, base);
};

// Jaiko: curly dark bob with a red artist beret on top
const styleBeret: HairFn = (buf, color) => {
  const [, base] = shades(color);
  for (let y = 6; y <= 15; y++) {
    set(buf, HX0 - 1, y, base); set(buf, HX0, y, base);
    set(buf, HX1, y, base); set(buf, HX1 + 1, y, base);
  }
  for (let x = HX0; x <= HX1; x++) set(buf, x, 6, base);
  const beret: RGB = [190, 35, 45];
  const [bhi, bbase, bsh] = shades(beret);
  rect(buf, 3, 2, 14, 4, bbase);
  rect(buf, 2, 3, 15, 4, bbase);
  for (let x = 4; x <= 13; x++) set(buf, x, 2, bhi);
  for (let x = 3; x <= 14; x++) set(buf, x, 5, bsh);
  set(buf, 8, 1, bbase); set(buf, 9, 1, bbase);
};

const HAIR_FNS = { styleShort, styleFloppy, styleFrame, styleBun, styleCurly, styleMessy, styleRecede, styleSpiky, styleBald, styleBowl, stylePigtails, styleSuneo, styleBeret };
...
// Nobita signature round wire-rim glasses
function drawRoundGlasses(buf: Buf): void {
  const frame: RGB = [32, 34, 40];
  const glint: RGB = [255, 255, 255];
  // Left circular rim (rows 7-11)
  for (const x of [4, 5, 6, 7]) { set(buf, x, 7, frame); set(buf, x, 11, frame); }
  for (let y = 8; y <= 10; y++) { set(buf, 3, y, frame); set(buf, 8, y, frame); }
  // Right circular rim (rows 7-11)
  for (const x of [9, 10, 11, 12]) { set(buf, x, 7, frame); set(buf, x, 11, frame); }
  for (let y = 8; y <= 10; y++) { set(buf, 8, y, frame); set(buf, 13, y, frame); }
  // Center nose bridge + side temples
  set(buf, 8, 8, frame); set(buf, 8, 9, frame);
  set(buf, 2, 9, frame); set(buf, 14, 9, frame);
  // Glass sheen
  set(buf, 4, 8, glint); set(buf, 9, 8, glint);
}

// ─── clothing ────────────────────────────────────────────────────────────────
type Cloth = 'suit' | 'dressshirt' | 'polo' | 'blouse' | 'cardigan' | 'sweater' | 'tshirt' | 'dress' | 'futuristic';
...
function drawClothing(buf: Buf, kind: Cloth, c1: RGB, c2: RGB | undefined, tie: RGB | undefined, skin: string, heavy = false): void {
  ...
  if (kind === 'suit') {
    ...
    for (const [x, y] of [[7, 19], [8, 19], [9, 19], [10, 19]] as const) set(buf, x, y, SKIN[skin].sh);
    if (c2) {
      const stripe = shades(c2)[1];
      for (let y = 22; y <= 24; y++) {
        for (let x = 2; x <= 15; x++) {
          if (alphaAt(buf, x, y)) set(buf, x, y, stripe);
        }
      }
    }
  } else if (kind === 'dress') {
    const white: RGB = [252, 250, 248];
    for (const [x, y] of [[6, 19], [7, 19], [10, 19], [11, 19], [7, 20], [10, 20]] as const) set(buf, x, y, white);
  } else if (kind === 'futuristic') {
    const neon: RGB = c2 ?? [0, 240, 255];
    for (let y = 19; y <= 27; y++) { set(buf, 8, y, neon); set(buf, 9, y, neon); }
    for (const [x, y] of [[4, 21], [13, 21], [3, 24], [14, 24]] as const) set(buf, x, y, neon);
  }
}
...
function drawSceneTorso(buf: Buf, r: Recipe, back: boolean): void {
  ...
  if (r.cloth === 'suit') {
    ...
    for (const [x, y] of [[7, 18], [8, 18], [9, 18], [10, 18]] as const) set(buf, x, y, skin.sh);
    if (r.c2) {
      const stripe = shades(r.c2)[1];
      for (let y = 20; y <= 22; y++) {
        for (let x = 3; x <= 14; x++) {
          if (alphaAt(buf, x, y)) set(buf, x, y, stripe);
        }
  ...
  } else if (r.cloth === 'dress') {
    const white: RGB = [252, 250, 248];
    for (const [x, y] of [[6, 18], [7, 18], [10, 18], [11, 18], [7, 19], [10, 19]] as const) set(buf, x, y, white);
  } else if (r.cloth === 'futuristic') {
    const neon: RGB = r.c2 ?? [0, 240, 255];
    for (let y = 18; y <= 24; y++) { set(buf, 8, y, neon); set(buf, 9, y, neon); }
  }
}
...
function drawDoraemonBody(buf: Buf, dorami = false, phase = 0, back = false): void {
  ...
  // Legs & Feet (rows 26-31)
  const leftLow = phase !== 1, rightLow = phase !== 2;
  rect(buf, 4, 26, 7, 28, bbase);
  rect(buf, 10, 26, 13, 28, bbase);
  rect(buf, 3, leftLow ? 29 : 28, 7, leftLow ? 31 : 30, white);
...
function composeDoraemon(dorami = false): Buf {
  ...
  const dark: RGB = [30, 26, 36];
  // Bust body
  rect(buf, 2, 18, 15, 27, blue);
  rect(buf, 4, 19, 13, 27, white);
  // Pocket arch
  for (let x = 6; x <= 11; x++) set(buf, x, 21, dark);
...
const RECIPES: Record<OfficeCharacterName, Recipe> = {
  // ─── Doraemon Universe ────────────────────────────────────────────────────
  doraemon: { skin: 'light', hairc: [28, 134, 238], hair: 'styleShort', cloth: 'suit', c1: [28, 134, 238] }, // Handled by custom composeDoraemon
  nobita:   { skin: 'light', hairc: [25, 25, 30],   hair: 'styleBowl',     cloth: 'polo', c1: [245, 215, 60], c2: [250, 250, 250], pants: [45, 65, 110], roundGlasses: true, brow: 'soft', mouth: 'smile' },
  shizuka:  { skin: 'light', hairc: [85, 45, 25],   hair: 'stylePigtails', cloth: 'dress', c1: [250, 110, 160], pants: [45, 60, 100], brow: 'soft', mouth: 'smile', blush: true, lashes: true },
  gian:     { skin: 'tan',   hairc: [30, 25, 25],   hair: 'styleSpiky',    cloth: 'tshirt', c1: [230, 115, 30], c2: [175, 75, 20], pants: [80, 85, 95], brow: 'angry', mouth: 'grin', heavy: true },
  suneo:    { skin: 'light', hairc: [25, 25, 30],   hair: 'styleSuneo',    cloth: 'polo', c1: [35, 135, 210], c2: [245, 245, 245], pants: [40, 130, 150], brow: 'raised', mouth: 'grin' },
  dorami:   { skin: 'light', hairc: [255, 214, 0],  hair: 'styleShort',    cloth: 'suit', c1: [255, 214, 0] }, // Handled by custom composeDoraemon(true)
  dekisugi: { skin: 'light', hairc: [75, 45, 25],   hair: 'styleShort',    hairargs: { part: 'L' }, cloth: 'sweater', c1: [35, 165, 95], pants: [140, 130, 115], brow: 'soft', mouth: 'smile' },
...
function drawHeadGroup(buf: Buf, r: Recipe): void {
  ...
  if (r.roundGlasses) drawRoundGlasses(buf);
}
