/**
 * Realtime Doraemon — voice toggle + live state indicator (card rt-3, Phase 1).
 *
 * A reusable mic button for the god/orchestrator agent ("Doraemon"). It consumes the
 * already-built `useRealtimeDoraemon()` voice-loop hook (a shared module-level singleton —
 ...
 * Doraemon is fullscreen). It is intentionally state-only / hook-only so both can mount it.
 */
...
import { useRealtimeDoraemon, type RealtimeStatus } from '@/realtime/session';
...
export interface RealtimeDoraemonToggleProps {
  /** Compact form for the fullscreen header / tight rows — hides the text label. */
  compact?: boolean;
}

export function RealtimeDoraemonToggle({ compact = false }: RealtimeDoraemonToggleProps) {
  const hasOpenAiKey = useStore((s) => s.hasOpenAiKey);
  const { status, error, connect, disconnect } = useRealtimeDoraemon();
