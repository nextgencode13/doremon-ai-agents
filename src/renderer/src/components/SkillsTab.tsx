...
export function SkillsTab({ agentCwd }: { agentCwd?: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%' }}>
      {/* Doraemon Secret Gadget Banner */}
      <div style={{
        padding: '6px 10px', background: 'var(--cth-sky-light)', borderBottom: '1px solid var(--cth-ink-300)',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 13 }}>🔔</span>
          <span style={{ fontFamily: 'var(--cth-font-display)', fontSize: 9, color: 'var(--cth-ink-900)', textTransform: 'uppercase' }}>
            22nd-Century Secret Gadgets (ひみつ道具)
          </span>
        </div>
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          <Chip text="🚪 Anywhere Door" tone="accent" />
          <Chip text="🍞 Memory Bread" tone="quiet" />
          <Chip text="🚁 Bamboo Copter" tone="quiet" />
          <Chip text="⏳ Time Machine" tone="quiet" />
        </div>
      </div>
      {/* Controls */}
      <div style={{
        ...
        <input
          ...
          placeholder={mode === 'installed' ? 'Search installed gadget skills…' : 'Search the 4D gadget catalog…'}
          style={{
