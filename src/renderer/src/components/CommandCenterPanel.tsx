...
/** Canonical tab order with Doraemon 4D gadget theme. */
const TABS: { key: CCTab; label: string; icon: Parameters<typeof Icon>[0]['name'] }[] = [
  { key: 'terminal', label: 'terminal', icon: 'terminal' },
  { key: 'floor', label: 'fleet monitor', icon: 'mcp' },
  { key: 'tasks', label: 'missions', icon: 'check' },
  { key: 'skills', label: 'gadgets (mcp)', icon: 'sparkle' },
  { key: 'memory', label: 'memory bread', icon: 'sparkle' },
  { key: 'graph', label: 'gadget graph', icon: 'web' },
  { key: 'human', label: 'ask me', icon: 'bell' },
  { key: 'triggers', label: 'time triggers', icon: 'clock' },
  { key: 'trigger-history', label: 'history', icon: 'ledger' },
  { key: 'activity', label: 'activity', icon: 'bell' },
  { key: 'workers', label: 'squad setup', icon: 'gear' }
];
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          width: 32, height: 32, background: 'var(--cth-sky-light)',
          boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)',
          display: 'flex', alignItems: 'flex-end', justifyContent: 'center', overflow: 'hidden', flexShrink: 0
        }}>
          <SpritePortrait character="doraemon" scale={1} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontFamily: 'var(--cth-font-display)', fontSize: 10, lineHeight: '14px', color: 'var(--cth-ink-900)',
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center', gap: 6
          }}>
            <span>DORAEMON COMMAND CENTER</span>
            <span style={{
              fontSize: 8, fontFamily: 'var(--cth-font-mono)',
              background: 'var(--cth-sky-light)', color: 'var(--cth-sky)',
              boxShadow: 'inset 0 0 0 1px var(--cth-sky)', padding: '0 4px', textTransform: 'uppercase'
            }}>4D POCKET</span>
          </div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 1, minWidth: 0 }}>
            ...
            }}>Doraemon 4D Command Center · Floor Lead: Nobita</span>
          </div>
        </div>
...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  const dispatch = async () => {
    ...
    setDispatchMsg(res.ok
      ? `sent to Doraemon${suggested ? ` (suggesting ${suggested.name})` : ''}`
      : `failed: ${res.error ?? '?'}`);
  ...
  return (
    <Scroll>
      <Section title="DISPATCH — VIA DORAEMON">
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          ...
          <Select value={dispatchTo} onChange={setDispatchTo}>
            <option value="">Doraemon decides</option>
        ...
        <textarea
          ...
          rows={2}
          placeholder="Describe the task… (Doraemon decomposes, writes the card, and assigns to the squad)"
          style={textareaStyle}
        />
