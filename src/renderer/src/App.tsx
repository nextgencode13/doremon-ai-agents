...
import { DoraemonBooting } from '@/components/DoraemonBooting';
...
export function App() {
  ...
  useEffect(() => {
    const legacyNames = ['doraemon', 'angela', 'kelly', 'jim', 'dwight', 'pam', 'kevin', 'stanley', 'phyllis', 'andy', 'creed', 'meredith', 'toby', 'ryan', 'oscar'];
  ...
  useEffect(() => {
    ...
    // Realtime Doraemon voice toggle can gate on it. Lives in the secret broker, not
  ...
  // hook) so Doraemon doesn't boot against the current home while the user may be
  ...
  // (Doraemon always spawns, so in normal operation it effectively never runs).
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      {/* rt-12: global fixed-overlay toast for voice-Doraemon completions ("Oscar
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minHeight: 0, minWidth: 0, position: 'relative' }}>
          <OfficeFloor />
          <MemoryPanel />
          {agentCount === 0 && godStatus === 'booting' && <DoraemonBooting />}
          ...
          {agent ? (
            ...
            <PixelPanel variant="default" noPadding style={{
              ...
              <p style={{ margin: 0, fontSize: 13, textAlign: 'center', color: 'var(--cth-ink-700)' }}>
                Doraemon is clocking in.<br />
                The terminal will land here once seated.
              </p>
            </PixelPanel>
          ) : (
            <PixelPanel variant="default" noPadding style={{
