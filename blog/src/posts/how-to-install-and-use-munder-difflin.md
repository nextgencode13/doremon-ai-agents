...
faq:
  ...
  - q: "Can I leave it running for hours or days?"
    a: "Yes — that's the point. With auto mode on, agents run unattended and Doraemon routes work and escalates only the critical calls to you. Give an agent a persistent Goal and it keeps working a long-horizon task across many prompts while you're away."
  - q: "Is auto mode safe?"
...
needs. Then you talk to <strong>Doraemon</strong>, spin up agents on any of <strong>ten
...
puts **Doraemon** in charge — a clone of you, the one agent *you* talk to. His card says **BOSS**:
...
asks Doraemon to fill the gaps for you.

{% img "note-1", "The Prerequisites page does this checklist for you — live status, real paths, and Doraemon on standby to install what's missing." %}
...
1. **Your clone's engine.** Doraemon runs on a pluggable engine; the card names all ten. Claude
...
Finish the wizard and you land on the office floor. Doraemon boots into his office automatically —
and everything the hive needs (message router, hook server, mission scheduler) starts with him,
on the very first run.

## Step 4: Meet Doraemon, your clone

Doraemon runs the floor: he triages requests, assigns work, and escalates only the critical calls
to you. He's the agent you talk to.

To talk to any agent (Doraemon included), select them on the floor to open their panel, then use the
...
Talking to Doraemon in plain language is how you steer the whole team: describe a goal, and he
...
Doraemon escalates genuinely critical actions (spending real money, destructive operations, big
...
1. **Give agents a persistent Goal.** The *Goal* field is injected into every prompt, so the agent
   ...
   after each."*
2. **Let Doraemon route the rest.** Tell him the high-level objective and let him assign sub-tasks.
...
4. **Walk away.** With auto mode on, the team keeps going unattended. Check the approvals queue
   when you're back; Doraemon only interrupts you for the critical calls.
...
## Step 9: Use Doraemon's Command Center

Select Doraemon on the floor and open his panel — a control surface, not a plain terminal:

- **Terminal** — Doraemon's live session, plus a message queue so you can park tasks while he works.
- **Floor** — the full agent roster with per-agent model selectors, a dispatch box, and your
  ...
  assistant who gathers context before Doraemon sees them.
...
- **Tasks** — a dependency-aware kanban board Doraemon and his team track status on.
...
pointed at Doraemon — *"Are all agents making progress? Re-engage anyone idle."* — keeps the team
