...
- **`POST`** to the endpoint with an `x-md-webhook-secret: <your-secret>` header and a JSON body of
  `{ message, title? }`. On success the message is routed to god/Doraemon, a stamped kanban card is created,
...
message to god/Doraemon** — the orchestrator's inbox — and **files a stamped kanban card**. From there it's
