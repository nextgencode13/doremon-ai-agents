...
const HOSTILE_PROMPT = [
  'You are "Doraemon" (god-1), an autonomous agent in a collaborating hive.',
...
test('cmd.exe: the same prompt is destroyed — which is the bug', () => {
  ...
  const atName = inner.indexOf('Doraemon');
  assert.ok(atName > 0);
...
test('the hive prompt survives a direct-executable shim, where cmd.exe destroys it', () => {
  const prompt = 'You are "Doraemon" (god-1), an autonomous agent.\n\nHIVE PROTOCOL\n- inbox\n- outbox';
