...
const TASKS = [
  ...
  { id: 'rt-2-realtime-session', title: 'Realtime Doraemon 2: WebRTC session — mic capture, VAD, barge-in', status: 'done' },
  { id: 'rt-3-toggle-ui', title: 'Realtime Doraemon 3: mic toggle + live state dot on AgentCard', status: 'done' },
...
// 6. ambiguity: "realtime doraemon" matches rt-2 AND rt-3 closely -> must flag, not guess
test('ambiguous: "realtime doraemon" flags disambiguation (no silent write)', () => {
  const r = find('realtime doraemon');
