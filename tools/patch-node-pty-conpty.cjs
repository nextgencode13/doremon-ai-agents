...
// Disable Spectre mitigation requirement in node-pty gyp files so building does not require optional Spectre MSVC packages
const gypFiles = [
  join(__dirname, '..', 'node_modules', 'node-pty', 'binding.gyp'),
  join(__dirname, '..', 'node_modules', 'node-pty', 'deps', 'winpty', 'src', 'winpty.gyp')
];
for (const gf of gypFiles) {
  if (existsSync(gf)) {
    const raw = readFileSync(gf, 'utf8');
    if (raw.includes("'SpectreMitigation': 'Spectre'")) {
      writeFileSync(gf, raw.replace(/'SpectreMitigation': 'Spectre'/g, "'SpectreMitigation': 'false'"), 'utf8');
      console.log(`[patch-node-pty-conpty] disabled SpectreMitigation in ${gf}`);
    }
  }
}
