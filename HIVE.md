...
3. **God-mode autonomy, native HITL.** A privileged **god agent** (lives in
   Doraemon's room) adjudicates cross-agent traffic. Routine requests
...
A fixed, always-on agent seated at `desk-ceo` (Doraemon's room), `character:
doraemon`, flagged `isGod`. It is an ordinary `claude` process — the *intelligence*
...
- **Phase 2 — God mode** ✅: the god agent auto-spawns into Doraemon's room
